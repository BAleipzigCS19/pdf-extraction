...
public final class Injector {
    ...
    private static Consumer<String> LOG_CONSUMER = getDefaultLogger();
    ...
    public static void setConfigurationSource(UnaryOperator<Object> configurationSupplier) {
    ...
    public static void injectMembers(Class<? extends Object> clazz, final Object instance) throws SecurityException {
        ...
        for (final Field field : fields) {
            if (field.isAnnotationPresent(Inject.class)) {
                LOG_CONSUMER.accept("Field annotated with @Inject found: " + field);
                Class<?> type = field.getType();
                String key = field.getName();
                Object value = configurator.getProperty(clazz, key);
                LOG_CONSUMER.accept("Value returned by configurator is: " + value);
                if (value == null && isNotPrimitiveOrString(type)) {
                    LOG_CONSUMER.accept("Field is not a JDK class");
                    value = instantiateModelOrService(type);
                }
                if (value != null) {
                    LOG_CONSUMER.accept("Value is a primitive, injecting...");
    ...
    @SuppressWarnings("java:S3011")//Injection definitely needs changing accessibility
    ...
    @SuppressWarnings("java:S3011")//Injection definitely needs this
    static void invokeMethodWithAnnotation(Class<?> clazz, final Object instance, final Class<? extends Annotation> annotationClass)
